...
class Groups extends Admin_Controller {
	...
	public function create()
	{
		...
		/* Validate form input */
		$this->form_validation->set_rules('group_name', 'Nome', 'required|alpha_dash');
		
		if ($this->form_validation->run() == TRUE)
		{
			$new_group_id = $this->ion_auth->create_group($this->input->post('group_name'), $this->input->post('description'),$this->input->post('cor'));
			if ($new_group_id)
			{
		...
		else
		{
			...
			$this->data['cor'] = array(
				...
				'type'  			=> 'text',
				'class' 			=> 'form-control',
				'value' 			=> $this->form_validation->set_value('cor','FFF')
			);		
	...
	public function deleteyes($id) {
		...
		$lixo = R::load("groups", $id);
		R::trash($lixo);

		$this->session->set_flashdata('message', 'Grupo excluido');	
	...
	public function profile($id)
	{
		...
		/* Data */
		$this->data['groups_ion']   = $groups;

		$grupo = array();
    	$grupo = (object)$grupo;

		foreach ($this->data['groups_ion'] as $k => $val)
		{							
			$grupo->$k = $val;
		}
		
		$this->data['groups']   = $grupo;
