...
$.fn.fullCalendar = function(options) {
	...
	this.each(function(i, _element) { // loop each DOM element involved
		...
		var calendar = element.data('fullCalendar'); // obter o objeto de calendário existente (se houver)
		var singleRes; // o valor retornado desta única chamada de método
		...
		if (typeof options === 'string') {
			if (calendar && $.isFunction(calendar[options])) {
				...
				if (!i) {
					res = singleRes; // gravar o primeiro resultado da chamada de método
				}
				if (options === 'destroy') { // para o método destroy, deve remover os dados do objeto Calendar
...
var complexOptions = [ // nomes de opções que são objetos cujas propriedades devem ser combinadas
	'header',
	'footer',
	'buttonText',
	'buttonIcons',
...
// Mescla uma matriz de objetos de opção em um único objeto
...
// Dadas as larguras da barra de rolagem de algum outro contêiner, crie bordas/margens em rowEls para corresponder à esquerda
// e espaço direito que foi compensado pelas barras de rolagem. Uma borda de 1 pixel primeiro, depois uma margem além disso.
...
// Desfaz compensaçãoScroll e restaura todas as bordas/margens
...
// Faça o cursor do mouse expressar que um evento não é permitido na área atual
function disableCursor() {
	$('body').addClass('fc-not-allowed');
}

// Retorna o cursor do mouse à sua aparência original
function enableCursor() {
	$('body').removeClass('fc-not-allowed');
}

// Dada uma altura total disponível para preencher, faça com que `els` (essencialmente linhas filhas) expanda para acomodar.
// Por padrão, todos os elementos menores que a altura recomendada são expandidos uniformemente, sem considerar
// quaisquer outros que já sejam muito altos. se `shouldRedistribute` estiver ativado, ele considera essas linhas altas e
// reduz a altura disponível.
function distributeHeight(els, availableHeight, shouldRedistribute) {

	// *NOTA DE PISO*: nós colocamos o piso em certos lugares porque o zoom pode fornecer dimensões de ponto flutuante imprecisas,
	// e é melhor ser mais baixo do que mais alto, para evitar criar barras de rolagem desnecessárias.
	var minOffset1 = Math.floor(availableHeight / els.length); // for non-last element
	var minOffset2 = Math.floor(availableHeight - minOffset1 * (els.length - 1)); // for last element *FLOORING NOTE*
	var flexEls = []; // elementos que podem se expandir. matriz de nós DOM
	var flexOffsets = []; // quantidade de espaço vertical que ocupa
	var flexHeights = []; // altura real do css
	var usedHeight = 0;

	undistributeHeight(els); // dar a todos os elementos sua altura natural

	// encontra elementos que estão abaixo da altura recomendada (expansível).
	// importante consultar alturas em uma única primeira passagem (para evitar oscilação de refluxo).	els.each(function(i, el) {
	els.each(function(i, el) {
		...
		if (naturalOffset < minOffset) {
			...
			//este elemento se estende além da altura recomendada (não expansível). marque o espaço como ocupado. 
			usedHeight += naturalOffset;
		}
	});

	// reajusta a altura recomendada para considerar apenas a altura disponível para linhas não maximizadas.
	...
	// atribui alturas a todos os elementos expansíveis
...
Grid.mixin({
	...
	//Utilitário para gerar propriedades CSS relacionadas ao tema do evento
...
var DayTableMixin = FC.DayTableMixin = {
	...
	computeColHeadFormat: function() {
		...
		//vários dias, portanto, a string de data única completa NÃO estará no texto do título
		else if (this.colCnt > 1) {
			return this.view.opt('dayOfMonthFormat'); // "Sat 12/10"
		}
		// único dia, então a string completa de data única provavelmente estará no texto do título
		else {
	...
	// Fatia um intervalo de datas em um segmento para cada linha da semana com a qual cruza
	sliceRangeByRow: function(range) {
		var daysPerRow = this.daysPerRow;
		var normalRange = this.view.computeDayRange(range); // fazer intervalo de dia inteiro, considerando nextDayThreshold
		var rangeFirst = this.getDateDayIndex(normalRange.start); // primeiro índice inclusivo
		var rangeLast = this.getDateDayIndex(normalRange.end.clone().subtract(1, 'days')); // inclusivo último índice
		var segs = [];
		var row;
		var rowFirst, rowLast; // intervalo de índice de dia inclusivo para a linha atual
		var segFirst, segLast; // intervalo de índice de dia inclusivo para o segmento

		for (row = 0; row < this.rowCnt; row++) {
			rowFirst = row * daysPerRow;
			rowLast = rowFirst + daysPerRow - 1;

			// intersectam o intervalo de deslocamento do segmento com o da linha
			segFirst = Math.max(rangeFirst, rowFirst);
			segLast = Math.min(rangeLast, rowLast);

			// lidar com índices intermediários
			segFirst = Math.ceil(segFirst); // no meio começa a rodada para a próxima célula
			segLast = Math.floor(segLast); // entre as extremidades arredondar para a célula anterior

			if (segFirst <= segLast) { // houve alguma interseção com a linha atual?
				segs.push({
					row: row,

					// normalizar para o início da linha
					firstRowDayIndex: segFirst - rowFirst,
					lastRowDayIndex: segLast - rowFirst,

					//deve corresponder a inteiros para ser o início/fim do segmento
	...
	// Fatia um intervalo de datas em um segmento para cada célula do dia com a qual cruza.
	// TODO: faça mais DRY com sliceRangeByRow de alguma forma.
...
DayGrid.mixin({
	...
	// Constrói o HTML a ser usado para o elemento padrão para um segmento individual
	fgSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		var isDraggable = view.isEventDraggable(event);
		...
		// Exiba apenas um horário de eventos cronometrados se for o segmento inicial
		if (seg.isStart) {
			timeText = this.getEventTimeText(event);
			if (timeText) {
				timeHtml =  '<span class="fc-time">' + htmlEscape(event.nome.split(' ').slice(0,1).join(' ')) + '</span>';//mostra o nome na view timetext
				//'<span class="fc-time">' + htmlEscape(timeText) + '</span>';
			}//split e slice é para pegar só o primeiro nome
		}
		// Aqui para aparecer na view
		titleHtml =
		'<span class="fc-title">' +
			(htmlEscape(event.hora || ' ') || '&nbsp;') + // mostra a hora na view
		'</span>';
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					(this.isRTL ?
						titleHtml + ' ' + timeHtml : // colocar um espaço natural entre
						timeHtml + ' ' + titleHtml   //
						) +
				'</div>' +
	...
	renderSegRow: function(row, rowSegs) {
		var colCnt = this.colCnt;
		var segLevels = this.buildSegLevels(rowSegs); //agrupar em sub-matrizes de níveis
		var levelCnt = Math.max(1, segLevels.length); // garantir pelo menos um nível
		var tbody = $('<tbody/>');
		var segMatrix = []; // pesquisa para quais segmentos são renderizados em quais células level+col
		var cellMatrix = []; // pesquisa para todos os elementos <td> da matriz level+col
		var loneCellMatrix = []; // pesquisa de elementos <td> que ocupam apenas uma única coluna
		var i, levelSegs;
		var col;
		var tr;
		var j, seg;
		...
		// preenche células vazias da coluna atual (`col`) para `endCol`
...
DayGrid.mixin({
	...
	//Cria o conteúdo interno do DOM do popover do segmento
	renderSegPopoverContent: function(row, col, segs) {
		var view = this.view;
		var isTheme = view.opt('theme');
		var title = this.getCellDate(row, col).format(view.opt('dayPopoverFormat'));
		//var nome = this.getCellDate(row, col).format(view.opt('dayPopoverFormat'));
		var content = $(
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	type: null, // nome da visão da subclasse (string)
	name: null, // descontinuada. use `type` em vez disso
	title: null, // o texto que será exibido no título do cabeçalho

	calendar: null, // objeto de calendário proprietário
	options: null, // hash contendo todas as opções. já mesclado com opções específicas de visualização
	el: null, // o elemento que contém a visão. definido pelo Calendário
	...
	// intervalo que a visualização está realmente exibindo (momentos)
	start: null,
	end: null, // exclusive

	// intervalo pelo qual a visão é formalmente responsável (momentos)
	// pode ser diferente de start/end. por exemplo, uma visualização de mês pode ter de 1º a 31º, excluindo datas preenchidas	intervalStart: null,
	intervalEnd: null, // exclusive
	intervalDuration: null,
	intervalUnit: null, // nome da maior unidade exibida, como "mês" ou "semana"

	isRTL: false,
	isSelected: false, // boolean se um intervalo de tempo é selecionado pelo usuário ou não
	selectedEvent: null,

	eventOrderSpecs: null, // critérios para ordenar eventos quando eles têm a mesma data/hora

	// classNames estilizados por temas aqui
	...
	// Aciona manipuladores relacionados à exibição. Modifica os argumentos antes de passar para o calendário.
	publiclyTrigger: function(name, thisObj) { // argumentos além de thisObj são passados ​​adiante
	...
	// Retorna um proxy da promessa dada que será rejeitada se o evento dado for acionado
	// antes que a promessa seja resolvida.
	...
	//Atualiza todas as datas internas para exibir o intervalo não zoneado fornecido.
	setRange: function(range) {
		$.extend(this, range); // atribui todas as propriedades às variáveis ​​de membro deste objeto
		this.updateTitle();
	},


	// Dada uma única data atual sem zoneamento, produza informações sobre qual intervalo exibir.
	// As subclasses podem substituir. Deve retornar todas as propriedades.
	computeRange: function(date) {
		...
		// normalizar a ambiguidade de tempo do intervalo
	...
	// Calcula a nova data quando o usuário pressiona o botão anterior, dada a data atual
	...
	// Calcula a nova data quando o usuário pressiona o próximo botão, dada a data atual
	...
	// Dada uma data atual do calendário calculada arbitrariamente, retorna uma data que é garantida como completamente
	// visível. `direction` é opcional e indica em qual direção a data atual estava sendo
	// incrementado ou decrementado (1 ou -1).
	massageCurrentDate: function(date, direction) {
		if (this.intervalDuration.as('days') <= 1) { // se a exibição exibir um único dia ou menos
	...
	// Define a propriedade de título da exibição para o valor calculado mais atualizado
	...
	// Gera a string de formato que deve ser usada para gerar o título para o intervalo de datas atual.
	// Tenta calcular o formato mais apropriado se não for especificado explicitamente com `titleFormat`.	
	...
	// Utilitário para formatar um intervalo. Aceita um objeto de intervalo, string de formatação e separador opcional.
	// Exibe intervalos de dia inteiro naturalmente, com um final inclusivo. Leva em consideração o isRTL atual.
	// Os fusos horários das datas dentro de `intervalo` serão respeitados.
	...
	// Gera HTML para uma âncora para outra visualização no calendário.
	// Gerará uma tag <a> ou uma tag <span> não clicável, dependendo das configurações habilitadas.
	// `gotoOptions` pode ser uma entrada momentânea ou um objeto com o formulário:
	// { data, tipo, forceOff }
	// `type` é um tipo de visualização como "day" ou "week". o valor padrão é "dia".
	// `attrs` e `innerHtml` são usados ​​para gerar o resto da tag HTML.
	buildGotoAnchorHtml: function(gotoOptions, attrs, innerHtml) {
		...
		if ($.isPlainObject(gotoOptions)) {
			...
			date = gotoOptions; // uma entrada de momento único
		}
		date = FC.moment(date); // se for uma string, analise-a

		finalOptions = { // para serialização no link
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2>&nbsp;</h2>')); // nós sempre queremos que ele ocupe altura
						isOnlyButtons = false;
					}
					else {
		...
		if (currentView) {
			...
			if (
				...
				if (elementVisible()) {
					...
					// precisa fazer isso depois de View::render, então as datas são calculadas
					// NOTA: visualize o texto do título das atualizações de forma proativa
...
Calendar.defaults = {
	...
	monthYearFormat: 'MMMM YYYY', // necessário para en. outras localidades dependem da opção computável do datepicker
